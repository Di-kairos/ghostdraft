...
> **Status: early (v0.1.17).** `pipe` (view without writing to disk) and `new` (a draft
...
> The current public release is **v0.1.17** (signed, with `install.sh` + `SHA256SUMS`).
> Pin it for reproducibility with `GHOSTDRAFT_VERSION=0.1.17` instead of `latest`.
