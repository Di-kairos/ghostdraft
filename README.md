...
> **Status: early (v0.1.11).** `pipe` (view without writing to disk) and `new` (a draft
...
> The current public release is **v0.1.11** (signed, with `install.sh` + `SHA256SUMS`).
> Pin it for reproducibility with `GHOSTDRAFT_VERSION=0.1.11` instead of `latest`.
