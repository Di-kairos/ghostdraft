...
> **Status: early (v0.1.15).** `pipe` (view without writing to disk) and `new` (a draft
...
> The current public release is **v0.1.15** (signed, with `install.sh` + `SHA256SUMS`).
> Pin it for reproducibility with `GHOSTDRAFT_VERSION=0.1.15` instead of `latest`.
...
setup) when `$EDITOR` is unset. To leave it: **`Ctrl-D`** saves and exits, **`Ctrl-X`** exits
