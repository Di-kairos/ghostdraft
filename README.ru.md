...
> **Статус: ранний (v0.1.10).** Готовы `pipe` (просмотр без записи на диск) и `new`
...
> Текущий публичный релиз — **v0.1.10** (подписан, с `install.sh` + `SHA256SUMS`).
> Для воспроизводимости фиксируй его: `GHOSTDRAFT_VERSION=0.1.10` вместо `latest`.
...
без `~/.viminfo`), если `$EDITOR` не задан. Чтобы выйти: нажми **`Esc`**, затем **`ZZ`** —
сохранить и выйти, или **`ZQ`** — выйти **без** сохранения (`:wq` / `:q!` тоже работают).
Внизу всегда висит подсказка. Клавиши **F2/F3** тоже назначены, но некоторые терминалы
(например Warp) их перехватывают до vim — поэтому надёжнее `Esc` → `ZZ`/`ZQ`.
