...
> **Статус: ранний (v0.1.15).** Готовы `pipe` (просмотр без записи на диск) и `new`
...
> Текущий публичный релиз — **v0.1.15** (подписан, с `install.sh` + `SHA256SUMS`).
> Для воспроизводимости фиксируй его: `GHOSTDRAFT_VERSION=0.1.15` вместо `latest`.
...
без `~/.viminfo`), если `$EDITOR` не задан. Чтобы выйти: **`Ctrl-D`** — сохранить и выйти,
